...
const Proposal: FC<Proposal> = (projectOrProposal) => {
	...
	const notify = useSiteNotify()
			...
			const [existing, setExisting] = useState<{project: boolean, name: boolean}>()
			...
			const setProject = useDelay<[Partial<Proposal>]>({
				...
				onEnd: () => {
					fetch(`https://${process.env.NEXT_PUBLIC_HOSTNAME}/api/proposal`, {
			...
			const select = () => {
				fetch(`https://${process.env.NEXT_PUBLIC_HOSTNAME}/api/proposal`, {
					method: 'GET',
					headers: makeHeaders(cleanObject({name, project}, (v) => v ) as {name?: string, project?: string})
				}).then(resp => {
					resp.json().then(data => {
						for (const i in map){
							map[i as keyof typeof map]?.(data[i])
						}
					})
				})
			}
			...
			return <div>
				...
				{existing?.project && editing === 'name' ? <input value={name} onChange={setValue('name')} /> : <span>{name}</span>}
